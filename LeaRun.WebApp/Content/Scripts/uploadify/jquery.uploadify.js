		...
		init : function(options, swfUploadOptions) {
			...
			return this.each(function() {
				...
				var settings = $.extend({
					...
					fileSizeLimit   : 9999,                  // The maximum size of an uploadable file in KB (Accepts units B KB MB GB if string, 0 for no limit)
			...
			if ($.inArray('onSelectError', settings.overrideEvents) < 0) {
				switch(errorCode) {
					...
					case SWFUpload.QUEUE_ERROR.FILE_EXCEEDS_SIZE_LIMIT:
                        //this.queueData.errorMsg += '\n文件 "' + file.name + '" 超过了限制的大小 (' + settings.fileSizeLimit + 'KB).';
                        this.queueData.errorMsg += '\n文件 "' + file.name + '" 超过了限制的大小 (4M).';
						break;
