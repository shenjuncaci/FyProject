...
{
    public class ProduceLineController : Controller
    {
        RepositoryFactory<FY_ProduceLine> repositoryfactory = new RepositoryFactory<FY_ProduceLine>();
        FY_ProduceLineBll PostBll = new FY_ProduceLineBll();
        //
        ...
        public ActionResult SubmitForm(string KeyValue, FY_ProduceLine entity, string BuildFormJson, HttpPostedFileBase Filedata,string DetailForm)
        {
            ...
            try
            {
                ...
                else
                {
                    ...
                    Base_DataScopePermissionBll.Instance.AddScopeDefault(ModuleId, ManageProvider.Provider.Current().UserId, entity.LineID, isOpenTrans);
                }
                Base_FormAttributeBll.Instance.SaveBuildForm(BuildFormJson, entity.LineID, ModuleId, isOpenTrans);
                database.Commit();
        ...
        {
            FY_ProduceLine entity = DataFactory.Database().FindEntity<FY_ProduceLine>(KeyValue);
            if (entity == null)
            {
                return Content("");
            }
