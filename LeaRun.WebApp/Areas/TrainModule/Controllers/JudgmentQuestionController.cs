...
{
    public class JudgmentQuestionController : Controller
    {
        RepositoryFactory<TR_JudgmentQuestion> repositoryfactory = new RepositoryFactory<TR_JudgmentQuestion>();
        TR_JudgmentQuestionBll JudgementQuestionBll = new TR_JudgmentQuestionBll();
        //
            ...
            string QuestionDescripe,string SkillType,string SkillID)
        {
            try
            {
                ...
                DataTable ListData = JudgementQuestionBll.GetPageList(keywords, ref jqgridparam, ParameterJson,QuestionDescripe, SkillType, SkillID);
                var JsonData = new
                {
        ...
        [HttpPost]
        public ActionResult SubmitForm(string KeyValue, TR_JudgmentQuestion entity, string BuildFormJson, HttpPostedFileBase Filedata)
        {
        ...
        {
            TR_JudgmentQuestion entity = DataFactory.Database().FindEntity<TR_JudgmentQuestion>(KeyValue);
            if (entity == null)
            {
                return Content("");
            }
        ...
        public void WriteLog(int IsOk, string KeyValue, string Message = "")
        {
            ...
            Base_SysLogBll.Instance.WriteLog<TR_JudgmentQuestion>(array, IsOk.ToString(), Message);
        }

        public ActionResult SkillJson()
        {
            string sql = " select SkillID,SkillName from TR_Skill where 1=1  ";
            DataTable dt = JudgementQuestionBll.GetDataTable(sql);
