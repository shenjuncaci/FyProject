...
{
    public class FY_CompetitorInfoBll : RepositoryFactory<FY_CompetitorInfoBll>
    {
        public DataTable GetPageList(string keyword, ref JqGridParam jqgridparam, string ParameterJson)
        {
            StringBuilder strSql = new StringBuilder();
            List<DbParameter> parameter = new List<DbParameter>();
            strSql.Append(@"select a.*,b.fy_cus_name,c.CompetitorName from FY_CompetitorInfo a 
left join fy_Cus b on a.cusid=b.fy_cus_id
left join fy_competitor c on a.competitorid=c.competitorid
where 1=1  ");
            if (!string.IsNullOrEmpty(keyword))
            {
                strSql.Append(@" AND (CompetitorName LIKE @keyword  or b.fy_cus_name like @keyword
                                    )");
