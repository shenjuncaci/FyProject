...
{
    public class FY_CompetitorBll : RepositoryFactory<FY_CompetitorBll>
    {
        public DataTable GetPageList(string keyword, ref JqGridParam jqgridparam, string ParameterJson)
        {
            StringBuilder strSql = new StringBuilder();
            List<DbParameter> parameter = new List<DbParameter>();
            strSql.Append(@"select * from FY_Competitor  where 1=1  ");
            if (!string.IsNullOrEmpty(keyword))
            {
                strSql.Append(@" AND (CompetitorName LIKE @keyword 
                                    )");
