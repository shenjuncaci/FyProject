...
{
    public class FY_CusInfoBll : RepositoryFactory<FY_CusInfoBll>
    {
        public DataTable GetPageList(string keyword, ref JqGridParam jqgridparam, string ParameterJson)
        {
            StringBuilder strSql = new StringBuilder();
            List<DbParameter> parameter = new List<DbParameter>();
            strSql.Append(@"select * from FY_CusInfo a left join fy_cus b on a.cusid=b.fy_cus_id where 1=1  ");
            if (!string.IsNullOrEmpty(keyword))
            {
                strSql.Append(@" AND (fy_cus_name LIKE @keyword or MessageType like @keyword
                                    )");
