    ...
    public class Base_ExcelImportBll : RepositoryFactory<Base_ExcelImport>
    {
        ...
        public int ImportExcel(string moduleId, DataTable dt, out DataTable Result)
        {
            ...
            else
            {
                ...
                //增加题库导入时的判断限制
                //if(tableName== "TR_ChoiceQuestion")
                //{
                ...
                if (dt != null && dt.Rows.Count > 0)
                {
                    ...
                    try
                    {
                        ...
                        foreach (DataRow item in dt.Rows)
                        {
                            #region 此处一般给导入的时候做一些格式限制，用tablename来个性化不同表的导入
                            if (tableName == "TR_ChoiceQuestion")
                            {
                                if(item["题目类型"].ToString()=="单选题")
                                {
                                    if(item["答案"].ToString()!="A"&& item["答案"].ToString() != "B" && item["答案"].ToString() != "C" && item["答案"].ToString() != "D" && item["答案"].ToString() != "E" && item["答案"].ToString() != "F")
                                    {
                                        DataRow dr = Newdt.NewRow();
                                        dr = Newdt.NewRow();
                                        dr[0] = errorNum;
                                        dr[1] = "第[" + rowNum.ToString() + "]行[单选题与答案格式要求不符合]";
                                        dr[2] = "格式不正确";
                                        Newdt.Rows.Add(dr);
                                        errorNum++;
                                        isExit = true;
                                ...
                                else if(item["题目类型"].ToString() == "多选题")
                                {
                                    //if (item["答案"].ToString() != "A" && item["答案"].ToString() != "B" && item["答案"].ToString() != "C" && item["答案"].ToString() != "D" && item["答案"].ToString() != "E" && item["答案"].ToString() != "F")
                                    //{
                                    //    DataRow dr = Newdt.NewRow();
                                    ...
                                    //    errorNum++;
                                    //}
                                    foreach(char c in item["答案"].ToString())
                                    {
                                        //StringBuilder sss = new StringBuilder();
                                        //sss.Append(c);
                                        //string b = sss.ToString();
                                        //int a = b.IndexOf("ABCDEF");
                                        if ("ABCDEF".IndexOf(c.ToString())<0)
                                        {
                                            DataRow dr = Newdt.NewRow();
                                            dr = Newdt.NewRow();
                                            dr[0] = errorNum;
                                            dr[1] = "第[" + rowNum.ToString() + "]行[多选题与答案格式要求不符合]";
                                            dr[2] = "格式不正确";
                                            Newdt.Rows.Add(dr);
                                            errorNum++;
                                            isExit = true;
                                ...
                                else
                                {
                                    ...
                                    dr = Newdt.NewRow();
                                    dr[0] = errorNum;
                                    dr[1] = "第[" + rowNum.ToString() + "]行[问题类型错误]";
                                    dr[2] = "格式不正确";
                                    Newdt.Rows.Add(dr);
                                    errorNum++;
                                    isExit = true;
