...
namespace LeaRun.Business
{
    ...
    public class Base_UserBll : RepositoryFactory<Base_User>
    {
        ...
        /// <returns></returns>
        public DataTable GetPageList(string keyword, string CompanyId, string DepartmentId, ref JqGridParam jqgridparam,string type)
        {
...
or dbo.fn_GetPinyin(RealName) like @keyword)");
            ...
            if(type=="1")
            {
                strSql.AppendFormat(" and exists (select * from Base_ObjectUserRelation where ObjectId='91c17ca4-0cbf-43fa-829e-3021b055b6c4' and UserId=T.UserId) ");
            }
            if(type=="2")
            {
                //重新调整，可以看到本部门下所有人员
                //strSql.AppendFormat(" and exists (select * from Base_ObjectUserRelation where ObjectId='f6afd4e4-6fb2-446f-88dd-815ddb91b09d' and UserId=T.UserId) ");
                strSql.AppendFormat(" and  DepartmentId='"+ManageProvider.Provider.Current().DepartmentId+ "' and exists (select * from Base_ObjectUserRelation where ObjectId='f6afd4e4-6fb2-446f-88dd-815ddb91b09d' and UserId=T.UserId)  ");
            }
            //客服列表
            if(type=="3")
            {
                strSql.AppendFormat(" and exists (select * from Base_ObjectUserRelation where ObjectId='9cc63534-b2cb-4cbb-99d9-50d43942ee47' and UserId=T.UserId) ");
            }
