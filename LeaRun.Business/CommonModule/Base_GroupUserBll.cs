...
namespace LeaRun.Business
{
    ...
    public class Base_GroupUserBll : RepositoryFactory<Base_GroupUser>
    {
        ...
        public DataTable GetPageListNew(string CompanyId, string DepartmentId, ref JqGridParam jqgridparam)
        {
            StringBuilder strSql = new StringBuilder();
            List<DbParameter> parameter = new List<DbParameter>();
            strSql.Append(@"SELECT  *
...
gu.StartTime,gu.EndTime
            ...
            if (!ManageProvider.Provider.Current().IsSystem)
            { 
                strSql.Append(" AND DepartmentId = '" + ManageProvider.Provider.Current().DepartmentId + "'");
        }
            //if (!string.IsNullOrEmpty(CompanyId))
            //{
            //    strSql.Append(" AND CompanyId = @CompanyId");
