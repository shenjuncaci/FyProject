...
{
    public class DM_CostAdjustBll : RepositoryFactory<DM_CostAdjust>
    {
        public DataTable GetPageList(string keyword, ref JqGridParam jqgridparam, string ParameterJson)
        {
            StringBuilder strSql = new StringBuilder();
            List<DbParameter> parameter = new List<DbParameter>();
            strSql.Append(@" select a.*,b.PersonName from DM_CostAdjust a left join DM_CheckIn b on a.UserID=b.CheckInID where 1=1 ");
            //if (!string.IsNullOrEmpty(keyword))
            //{
            //    strSql.Append(@" AND (AssetName LIKE @keyword
            //                        )");
